...
float32_t out_temp=0.0;

/* Global variables Def */
float32_t max_output=0.0;
...
int main(void) {
	...
	while (1) {
		...
		for(max_output_index=0;max_output_index<FFT_SIZE/2;max_output_index++)
		{
			if(Output[max_output_index]>max_output)
			{
				max_output=Output[max_output_index];
				max_k = max_output_index;
			}
		}
